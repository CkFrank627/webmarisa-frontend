...
func Configure(app *iris.Application) {
	...
	app.PartyFunc("/", func(r iris.Party) {
		r.Post("Add", Controllers.AuditUserInput, hero.Handler(Controllers.Add))
		r.Post("Reply", Controllers.AuditUserInput, hero.Handler(Controllers.Reply))
		r.Post("Forget", Controllers.AuditUserInput, hero.Handler(Controllers.Forget))
	...
	app.PartyFunc("/api", func(r iris.Party) {
		...
		r.Get("/conversations", hero.Handler(Controllers.ListConversations))
		r.Put("/conversations", Controllers.AuditUserInput, hero.Handler(Controllers.SyncConversations))
		r.Get("/messages", hero.Handler(Controllers.ListMessages))
		r.Post("/messages", Controllers.AuditUserInput, hero.Handler(Controllers.PostMessage))
	...
	app.PartyFunc("/api/custom", func(r iris.Party) {
		...
		r.Post("/personas", Controllers.AuditUserInput, hero.Handler(Controllers.CreatePersona))
		...
		r.Post("/personas/{id:uint64}/teach", Controllers.AuditUserInput, hero.Handler(Controllers.AddPersonaTeach))
		r.Delete("/personas/{id:uint64}/teach/last", hero.Handler(Controllers.DeleteTeachLast))

		r.Post("/reply", Controllers.AuditUserInput, hero.Handler(Controllers.CustomReply))
	...
	app.PartyFunc("/api/classic", func(r iris.Party) {
		...
		r.Put("/{id:int64}", Controllers.AuditUserInput, hero.Handler(Controllers.ClassicUpdate))
		r.Get("/list", hero.Handler(Controllers.ClassicList)) // ✅ 新增
		r.Post("/reply", Controllers.AuditUserInput, hero.Handler(Controllers.ClassicReply))
		r.Post("/teach", Controllers.AuditUserInput, hero.Handler(Controllers.ClassicTeach))
		r.Post("/forget", Controllers.AuditUserInput, hero.Handler(Controllers.ClassicForget))
	})
